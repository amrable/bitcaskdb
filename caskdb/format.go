...
func EncodeHeader(timestamp uint32, keySize uint32, valueSize uint32) []byte {
	...
	b := make([]byte, 4)
	c := make([]byte, 4)
	binary.BigEndian.PutUint32(a, timestamp)
	binary.BigEndian.PutUint32(b, keySize)
	binary.BigEndian.PutUint32(c, valueSize)
...
func EncodeKV(timestamp uint32, key string, value string) (int, []byte) {
	...
	b := []byte(key)
	c := []byte(value)
	a := EncodeHeader(timestamp, uint32(len(b)), uint32(len(c)))

	return size, append(append(a, b...), c...)
}

func DecodeKV(data []byte) (uint32, string, string) {
	headerSlice := data[:HeaderSize]
	timestamp, keySize, valueSize := DecodeHeader(headerSlice)
